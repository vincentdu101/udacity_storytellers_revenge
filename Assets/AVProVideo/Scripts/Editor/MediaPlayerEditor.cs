	...
	public class MediaPlayerEditor : UnityEditor.Editor
	{
		...
		private void OnInspectorGUI_Override_Windows()
		{
			...
			if (propVideoApi != null)
			{
				...
				if (propForceAudioResample.boolValue)
				{
					GUI.enabled = false;
				}

				SerializedProperty propDesiredAudioChannels = serializedObject.FindProperty(optionsVarName + ".desiredAudioChannels");
				if (propDesiredAudioChannels != null)
				{
					EditorGUILayout.PropertyField(propDesiredAudioChannels, new GUIContent("Audio Channels", "Number of audio channels to read. Must be either equal to, or greater than the actual number of audio channels in the video"));
				}
