...
namespace RenderHeads.Media.AVProVideo
{
	public sealed partial class WindowsMediaPlayer : BaseMediaPlayer
	{
		...
		private static int CalculateClosestViableChannelNumber(int channels)
		{
			...
			channels = Mathf.Clamp(channels, 1, maxChannels);

			int[] viableChannels = {8, 6, 5, 4, 2, 1};

			for(int i = 0; i < viableChannels.Length; ++i)
			{
				if(channels >= viableChannels[i])
				{
					return viableChannels[i];
				}
			}
		...
		public WindowsMediaPlayer(Windows.VideoApi videoApi, bool useHardwareDecoding, bool useTextureMips, bool useLowLatency, string audioDeviceOutputName, bool useUnityAudio, bool forceResample, int channels)
		{
			SetOptions(videoApi, useHardwareDecoding, useTextureMips, useLowLatency, audioDeviceOutputName, useUnityAudio, forceResample, channels);
		}

		public void SetOptions(Windows.VideoApi videoApi, bool useHardwareDecoding, bool useTextureMips, bool useLowLatency, string audioDeviceOutputName, bool useUnityAudio, bool forceResample, int channels)
		{
			_videoApi = videoApi;
		...
		public override bool OpenVideoFromFile(string path, long offset, string httpHeaderJson)
		{
			...
			int channelsForAudio = CalculateClosestViableChannelNumber(_desiredAudioChannels);

			_instance = Native.OpenSource(_instance, path, (int)_videoApi, _useHardwareDecoding, _useTextureMips, _useLowLatency, _audioDeviceOutputName, _useUnityAudio, _forceAudioResample, channelsForAudio);
		...
		public override bool OpenVideoFromBuffer(byte[] buffer)
		{
			...
			_instance = Native.OpenSourceFromBuffer(_instance, buffer, (ulong)buffer.Length, (int)_videoApi, _useHardwareDecoding, _useTextureMips, _useLowLatency, _audioDeviceOutputName, _useUnityAudio);
		...
		public override bool OpenVideoFromFile(IRandomAccessStream ras, string path, long offset, string httpHeaderJson)
		{
			...
			_instance = Native.OpenSourceFromStream(_instance, ras, path, (int)_videoApi, _useHardwareDecoding, _useTextureMips, _useLowLatency, _audioDeviceOutputName, _useUnityAudio, _forceAudioResample, channelsForAudio);
		...
		private struct Native
		{
			...
			[DllImport("AVProVideo")]
			public static extern System.IntPtr OpenSource(System.IntPtr instance, [MarshalAs(UnmanagedType.LPWStr)]string path, int videoApiIndex, bool useHardwareDecoding, bool generateTextureMips, bool useLowLatency, [MarshalAs(UnmanagedType.LPWStr)]string forceAudioOutputDeviceName, bool useUnityAudio, bool forceResample, int channels);

			[DllImport("AVProVideo")]
			public static extern System.IntPtr OpenSourceFromBuffer(System.IntPtr instance, byte[] buffer, ulong bufferLength, int videoApiIndex, bool useHardwareDecoding, bool generateTextureMips, bool useLowLatency, [MarshalAs(UnmanagedType.LPWStr)]string forceAudioOutputDeviceName, bool useUnityAudio);
